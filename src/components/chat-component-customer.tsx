...
export default function ChatComponent({ onBackButtonClick }: ComponentProps) {
  ...
  async function connectWebSockets(msg: string) {
    try {
      ...
      if (socketRef.current) {
        try { socketRef.current.close() } catch {console.error("Couldn't close socket connection")}
  ...
  useEffect(() => {
    return () => {
      ...
      if (socketRef.current) {
        try { socketRef.current.close() } catch { console.error("An error occured, couldn't close websocket connection.")}
        socketRef.current = null
      }
    }
  }, [])
  ...
  const handleGetMoreHelp = async () => {
    ...
    try {
      ...
      if (thread.length === 0) { throw Error("An error occured, couldn't create chat thread")}
      
      currentThreadIdRef.current = thread;
    } catch (error) {
      if (error instanceof Error) {
        toast.error("An error occured when trying to create a chat thread: " + error.message);
      } else {
        toast.error("An unknown error occured when trying to create a chat thread")
      }
    }
  }
              ...
              <ChatBubble key={key} variant={isSent ? "sent" : "received"}>
                <ChatBubbleAvatar>
                  <ChatBubbleAvatarImage
                    src={isSent ? userData.userData.name : agentData.img}
                    alt={isSent ? userData.userData.email : agentData.username}
                  />
                </ChatBubbleAvatar>
